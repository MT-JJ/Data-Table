###Function requires dataset with all matching covariates and categorical variables entered as binary with the reference variables specified in function argument. Results include all variables but glm is run by excluding ref variables to avoid dummy variable trap.
PSM_diagnostics<-function(Matchingcovs,Treatvar,ID_vars,convert_ind=T,ref_vars=NULL,missing_check=F,family="binomial",plot.points=T,pretty_print=F,add_new_file=T,psm=T,global=T,...){
  ...
  if(missing_check!=F){
    ...
    Matchingcovs<-Impute_missing_fn(Matchingcovs,impute_method = impute_method,ignore_vars = c(ID_vars,Treatvar))
  }
  ...
  if(is.null(ref_vars)){
    if(convert_ind){##No ref vars provided but user wants to convert##
      tmp_X<-data.table(Matchingcovs)[,-c(ID_vars,Treatvar),with=F][,lapply(copy(.SD),function(i) if(is.character(i)|is.factor(i)){as.factor(i)} else{as.numeric(i)})]
  ...
  Treatvar<-make.names(Treatvar)
  ll<-lapply(as.list(match.call(expand.dots = F))[c("Treatvar","ID_vars")],as.name)
    
  if(is.numeric(X[[Treatvar]])){
    print(paste(sQuote(match.call()$Treatvar),"is not a factor.  Assuming",sQuote(max(X[[Treatvar]])),"is the treatment group."));X[[Treatvar]]<-factor(plyr::mapvalues(X[[Treatvar]],sort(unique(X[[Treatvar]])),c("Comparison","Treatment")))} else{X[,eval(Treatvar):=lapply(.SD,factor),.SDcols=Treatvar]
    }
  Treatvarlevels<-setNames(1:2,levels(X[[Treatvar]]))
  if(sum(grepl("comp|treat|control",tolower(levels(X[[Treatvar]])))==F)!=0){
    print(paste("For assigning inverse weights, assuming first level is the comparison and second level is treatment (i.e.,",sQuote(levels(X[[Treatvar]])[1]),"=","comparison",";",sQuote(levels(X[[Treatvar]])[2]),"=","treatment). If this is incorrect either recode factor and re-run or manually change inverse weight."))
    }
  ...
  if(psm==T){
    ...
    if(is.null(ref_vars)){
      ...
      tmp_data<-data.table(data.frame(glmtest$model,psmscore=predict(glmtest,type="response")))[,Inverse_weight:=ifelse(as.numeric(eval(ll$Treatvar))==1,1/(1-psmscore),1/psmscore)]
      
    } else {
      
      tmp_data<-data.table(data.frame(glmtest$model,psmscore=predict(glmtest,type="response")))[,Inverse_weight:=ifelse(as.numeric(eval(ll$Treatvar))==1,1/(1-psmscore),1/psmscore)][Matchingcovs[,c(ID_vars,ref_vars),with=F],on=ID_vars]
    }
    ...
    if(nrow(glmtest$model)!=nrow(glmtest$data)){
      ...
      if(psm==F){final_datapsm<-copy(X)} else{
      final_datapsm<-setnames(copy(X),psm,"psmscore")[,Inverse_weight:=ifelse(as.numeric(eval(ll$Treatvar))==1,1/(1-psmscore),1/psmscore)]
      }
  }
  ...
  Group_means_sds<-IndicatorNs[eval(setnames(StatsDT_fn(copy(final_datapsm)[,-c("Inverse_weight"),with=F],Treatvar,ID=ID_vars,group_col = F,stat_col = F,round_digit=4,percents=F),"variable","Covariates")),on="Covariates"][is.na(N_Treatment),c("N_Treatment","N_Comparison"):=list(i.N_Treatment,i.N_Comparison)]
  ...
  if(psm!=F){
    ...
    PSM_All<-copy(final_datapsm)[is.na(psmscore)==F,list(N=.N,Mean=mean(psmscore,na.rm=T),SD=sd(psmscore,na.rm=T),Median=median(psmscore,na.rm=T),Range=(paste0(format(round(range(psmscore,na.rm=T),4),nsmall = 4),collapse = "-")))]
      
    PSM_Treat<-copy(final_datapsm)[is.na(psmscore)==F,list(N=.N,Mean=mean(psmscore,na.rm=T),SD=sd(psmscore,na.rm=T),Median=median(psmscore,na.rm=T),Range=(paste0(format(round(range(psmscore,na.rm=T),4),nsmall = 4),collapse = "-"))),by=Treatvar][,c(Treatvar,"Range"):=lapply(.SD,as.character),.SDcols=c(Treatvar,"Range")]
  ...
  if(pretty_print==T){
    ...
    if(add_new_file==T){
      Pretty_excel_print(copy(Group_means_sds)[,-grep("^i.",colnames(Group_means_sds)),with=F],var_col = "Covariates",grouped_col = 2,filename = tmpfile ,sheetname=tmpsheet)
        } else {
          Pretty_excel_print(copy(Group_means_sds)[,-grep("^i.",colnames(Group_means_sds)),with=F],var_col = "Covariates",grouped_col = 2,filename = tmpfile,sheetname=tmpsheet,append = T)
        }
    invisible(Pretty_excel_print(copy(final_results)[,c("Pooled_SD","SD_mean_diff"):=lapply(.SD,function(i) round(i,4)),.SDcols=c("Pooled_SD","SD_mean_diff")],var_col = "Covariates",grouped_col = 2,filename = paste0("Descriptive Stats for ",dataframe_name,".xlsx"),sheetname=tmpsheet2,append = T))
  }
