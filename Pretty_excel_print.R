Pretty_excel_print<-function(stat_dt,grouping_col=NULL,stat_col=NULL,var_col=NULL,grouped_col=NULL,filename=NULL,sheetname=NULL,append=F,...){
  ...
  if(Var_colshade_ind=="Neither"){
    switch(Stat_group_ind,
           ...
           stop("Insufficient information to group columns. Either use the 'Create Excel' function to print as is or enter the number of columns in each group."))} else if(Var_colshade_ind=="Var_no_colshade"){
           ...
           } else if(Var_colshade_ind=="Var_colshade"){
             switch(Stat_group_ind,
                    "FALSE.FALSE"={
                      ll<-lapply(as.list(match.call())[c("stat_dt","grouping_col","stat_col","var_col")],as.name)
                      group_pattern<-X[,rle(eval(ll$grouping_col))$length]
