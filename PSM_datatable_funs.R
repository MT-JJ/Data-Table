...
Optimal_match_fn<-function(Matchingcovs_PSM,Treatvar,ID_vars,ratio=1,fitted_glm=T,glm_family="binomial",glmweights=NULL,other_fitted_model=NULL,custom_dist=NULL,Match_type=c("both","pair","full"),exact_vars=NULL,include_exact_psm=F,caliper=NULL,Caliper_vars=NULL,Caliper_glm=T,dist_method=c("euclidean","mahalanobis","rank_mahalanobis","custom"),standardization.scale=sd,min.controls = 0, max.controls = Inf,omit.fraction=NULL,global=T){
  ...
  X_optmatch<-copy(X)[,eval(Treatvar):=switch(Treatvar_class,
                                              "factor"={
                                                print(paste0(sQuote(ll$Treatvar)," is a factor. Assigning 0 to ",sQuote(levels(X[[Treatvar]])[1])," and 1 to ",sQuote(levels(X[[Treatvar]])[2]),". If this is incorrect assign higher level of ",sQuote(ll$Treatvar)," to the treatment and rerun."));as.numeric(X[[Treatvar]])-1
                                                },
                                              "numeric"={
                                                print(paste0(sQuote(ll$Treatvar)," is numeric. Assigning 0 to ",sQuote(min(X[[Treatvar]]))," and 1 to ",sQuote(max(X[[Treatvar]])),". If this is incorrect assign higher value of ",sQuote(ll$Treatvar)," to the treatment and rerun."));plyr::mapvalues(X[[Treatvar]],sort(unique(X[[Treatvar]])),c(0,1))
                                                },
                                              "character"={
                                                tmp_fac<-factor(ll$Treatvar)
                                                print(paste0(sQuote(X[[Treatvar]])," is a character vector. Assigning 0 to ",sQuote(levels(tmp_fac)[1])," and 1 to ",sQuote(levels(tmp_fac)[2]),". If this is incorrect convert ",sQuote(ll$Treatvar)," to a factor or numeric variable and rerun."))
                                                as.numeric(tmp_fac)-1
                                                },
                                              stop(print(paste("Cannot match on",sQuote(ll$Treatvar),"because it is not a factor or numeric vector.  Check to see if you assigned correct variable to",sQuote("Treatvar"))))
                                              )]
  ...
  if(!is.list(final_matches_all)){
    if(length(matches)==2){
      ...
      final_matches_all<-tryCatch(lapply(matches["Full"],function(i) data.table(data.frame(X,Match_num=as.numeric(plyr::mapvalues(i,levels(i),1:nlevels(i)))))),error=function(e) stop("No treatment units could be matched using optimal pair or full matching. Try loosening your criteria and/or try another matching method.",call. = F))
        ...
        stop(paste("No treatment units could be matched using", Match_type[1], "match. Try loosening your criteria and/or try another matching method."))
    }
    }
