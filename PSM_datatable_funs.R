...
Nearest_Match_fn<-function(Matchingcovs_PSM,Treatvar,ID_vars,include.psm=F,distance="logit",custom_dist=F,matching_model="glm",family="binomial",weights=NULL,ratio=1,m.order="random",caliper=NULL,exact_vars=NULL,ties=T,distance.tolerance=1e-05,global=T,...){
  ...
  if(is.null(exact_vars)){
    ...
    if(is.list(Exact_all_test)){
      ...
      Final_matched_sets<-append(Final_matched_sets,list(Exact=unique(exact_patterns[eval(copy(X)[exact_data_IDs,on="ID"][Exact_all,on="ROWID"][,-c("ROWID"),with=F]),on=colnames(exact_data)],by="ID")))
...
Optimal_match_fn<-function(Matchingcovs_PSM,Treatvar,ID_vars,ratio=1,fitted_glm=T,glm_family="binomial",glmweights=NULL,other_fitted_model=NULL,custom_dist=NULL,Match_type=c("both","pair","full"),exact_vars=NULL,include_exact_psm=F,caliper=NULL,Caliper_vars=NULL,Caliper_glm=T,dist_method=c("euclidean","mahalanobis","rank_mahalanobis","custom"),standardization.scale=sd,min.controls = 0, max.controls = Inf,omit.fraction=NULL,global=T){
  ...
  Treatvar_class<-class(X[[Treatvar]])
  X_optmatch<-copy(X)[,eval(Treatvar):=switch(Treatvar_class,
                        "factor"={print(paste0(sQuote(ll$Treatvar)," is a factor. Assigning 0 to ",sQuote(levels(X[[Treatvar]])[1])," and 1 to ",sQuote(levels(X[[Treatvar]])[2]),". If this is incorrect assign higher level of ",sQuote(ll$Treatvar)," to the treatment and rerun."));as.numeric(X[[Treatvar]])-1
                        },
                        "numeric"={print(paste0(sQuote(ll$Treatvar)," is numeric. Assigning 0 to ",sQuote(min(X[[Treatvar]]))," and 1 to ",sQuote(max(X[[Treatvar]])),". If this is incorrect assign higher value of ",sQuote(ll$Treatvar)," to the treatment and rerun."));plyr::mapvalues(X[[Treatvar]],sort(unique(X[[Treatvar]])),c(0,1))  
                        },
...
X_optmatch<-suppressWarnings(X_optmatch[,-c("psmscore"),with=F])
#Match_on syntax
  ...
  glm_method<-switch(dist_method[1],
                 "euclidean"={
                   if(!include_exact_psm){substitute(glm(paste(eval(Treatvar),"~",paste0(colnames(X_optmatch)[-match(c(Treatvar,ID_vars,exact_vars),colnames(X_optmatch))],collapse = "+")),data=X_optmatch,family=glm_family,weights=glmweights))} else {substitute(glm(as.formula(paste(Treatvar,"~",paste0(colnames(X_optmatch)[-match(c(Treatvar,ID_vars),colnames(X_optmatch))],collapse = "+"))),data=X_optmatch,family=glm_family,weights=glmweights))}
                   },
                 "mahalanobis"={
                 ...
                 "custom"={if(is.null(custom_dist)){
                   stop(paste0("Entering custom implies you created your own distance measure outside this function and attached that measure to your data set.  Please include the name of this variable in ",dataframe_name,"."))} else{
  ...
  match_on_matrix<-switch(exact_cal_var_ind,
                          "TRUE.TRUE"={if(is.null(custom_dist)) {match_on(eval(glm_method),data=X_optmatch,caliper=caliper,standardization.scale=standardization.scale)} else{match_on(x=glm_method$x,data=X_optmatch,z=glm_method$z,caliper=caliper,standardization.scale=standardization.scale)}
                          },
                          "TRUE.FALSE"={if(is.null(custom_dist)) {
                            match_on(eval(glm_method),data=X_optmatch,standardization.scale=standardization.scale)+Caliper_model} else {
                              match_on(x=glm_method$x,data=X_optmatch,z=glm_method$z,standardization.scale=standardization.scale)+Caliper_model}
                            },
                           "FALSE.TRUE"={if(is.null(custom_dist)) {
                           ...
                           "FALSE.FALSE"={if(is.null(custom_dist)) {
  ...
  matches<-switch(Match_type[1],
                  ...
                  "pair"={
                    pair(match_on_matrix,controls = ratio,data=X_optmatch)
                  },
                  "full"={
                    full(match_on_matrix,data=X_optmatch,min.controls=min.controls,max.controls=max.controls,omit.fraction=omit.fraction)
                  },
                  stop("Must enter a valid type of matching!"))

  final_matches_all<-tryCatch(
    {lapply(matches,function(i) data.table(data.frame(X,Match_num=as.numeric(plyr::mapvalues(i,levels(i),1:nlevels(i))))))
    },error=function(e) 1)
  
  if(!is.list(final_matches_all)){
    if(length(matches)==2){
      print("No matches resulted from optimal pair matching.")
      final_matches_all<-tryCatch(lapply(matches["Full"],function(i) data.table(data.frame(X,Match_num=as.numeric(plyr::mapvalues(i,levels(i),1:nlevels(i)))))),error=function(e) stop("No treatment units could be matched using optimal pair or full matching. Try loosening your criteria and/or try another matching method.",call. = F))
    } else {stop(paste("No treatment units could be matched using", Match_type[1], "match. Try loosening your criteria and/or try another matching method."))}
  }
    final_matches<-lapply(final_matches_all,function(i) i[is.na(Match_num)==F])
    if(global){
  ...
  Unmatched_treat<-lapply(final_matches_all,function(i) data.table(data.frame(ID=unname(unlist(copy(i)[eval(ll$Treatvar)==1&is.na(Match_num),ID_vars,with=F])))))
  
  ###Printing quick summary to console
  lapply(seq_along(final_matches),function(i) print(paste(nrow(Unmatched_treat[[i]]),"treatment subjects removed and",nrow(unique(copy(final_matches[[i]]),by=ID_vars)),"total unique subjects in the",names(final_matches)[i],"matched set.")))
