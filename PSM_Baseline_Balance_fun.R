...
Baseline_equiv_fn<-function(MatchedSets,Treatvar,ID_vars,pre_pooled_sd,Match_num="Match_num",SD_mean_before=NULL,ref_var=NULL,other_psm=NULL,plot.points=T,outlier_dist=3,global=T){
  ...
  if(is.numeric(MatchedSets[[Treatvar]])){
    ...
    X[[Treatvar]]<-factor(plyr::mapvalues(X[[Treatvar]],sort(unique(X[[Treatvar]])),c("Comparison","Treatment")))
  } else{
  ...
  if(global){
    ...
    assign(paste("Compare",dataname,sep="_"),tryCatch(setnames(SD_mean_before[copy(final_results)[,c("Covariates","SD_mean_diff"),with=F],on="Covariates"][,c("SD_mean_diff","i.SD_mean_diff"):=lapply(.SD,function(i) format(round(i,4),nsmall=4)),.SDcols=c("SD_mean_diff","i.SD_mean_diff")],c("Covariates","SD_means_before_matching","SD_means_after_matching")),error=function(e) {
  ...
  if(Compare_ind){
    ...
    return(list(Full_data_wts=Weighted_covariates,Matched_data_wts=Weighted_covariates_graph,Descriptive_stats_wts=Group_means_sds,All_calc=final_results[,colnames(final_results)[-match("Covariates",colnames(final_results))]:=lapply(.SD,function(i) round(i,4)),.SDcols=colnames(final_results)[-match("Covariates",colnames(final_results))]],Compare=tryCatch(setnames(SD_mean_before[copy(final_results)[,c("Covariates","SD_mean_diff"),with=F],on="Covariates"],c("Covariates","SD_means_before_matching","SD_means_after_matching")),error=function(e) NULL),PSM_summary=PSM_summary,All_PSMmatches=Matched_dist_comparisons,Outliers_only=Outlier_only,PSM_Hist=PSM_hist_after))
  } else {
